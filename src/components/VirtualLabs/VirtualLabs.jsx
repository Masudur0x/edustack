...
import {
  FlaskConical, Clock, Users, ChevronRight, ChevronLeft, Play, X,
...
function LabCard({ lab, onLaunch, completed }) {
  ...
  return (
    <div className="card-hover flex flex-col group">
      ...
      <p className="text-xs text-white/55 mb-3 line-clamp-2 flex-1">{lab.objective}</p>

      <div className="flex items-center gap-3 mb-3 text-xs text-white/55">
...
function CodeEditor({ lab }) {
  ...
  const run = async () => {
    ...
    const outputs = {
      ...
      ds: `   age  salary  dept\n0   28   55000    IT\n1   34   72000    HR\n\nage      0\nsalary   1\ndtype: int64\n\n[ok] Dataset loaded, 1 missing value found in 'salary'`,
      default: `[ok] Code executed successfully\nOutput: Lab step complete`,
    }
    ...
    ee: {
      ...
      visual: (
        <svg viewBox="0 0 400 200" className="w-full h-40 bg-[#0f1014] rounded-lg p-4">
          ...
          <text x="115" y="64" fill="#f59e0b" fontSize="9">100{'Ω'}</text>
          <rect x="200" y="50" width="60" height="20" rx="3" fill="none" stroke="#f59e0b" strokeWidth="2" />
          <text x="215" y="64" fill="#f59e0b" fontSize="9">220{'Ω'}</text>
          <rect x="300" y="50" width="60" height="20" rx="3" fill="none" stroke="#f59e0b" strokeWidth="2" />
          <text x="315" y="64" fill="#f59e0b" fontSize="9">330{'Ω'}</text>
          <line x1="20" y1="100" x2="380" y2="100" stroke="#22d3ee" strokeWidth="2" />
          <line x1="380" y1="60" x2="380" y2="100" stroke="#22d3ee" strokeWidth="2" />
          <text x="130" y="130" fill="#a3e635" fontSize="10">Total R = 650{'Ω'}</text>
          <text x="130" y="148" fill="#a3e635" fontSize="10">I = 9V {'÷'} 650{'Ω'} = 13.8mA</text>
        </svg>
      ),
    },
    math: {
      desc: 'Visualize the derivative of f(x) = x² as a tangent line.',
      visual: (
        <svg viewBox="0 0 400 200" className="w-full h-40 bg-[#0f1014] rounded-lg p-4">
          ...
          <text x="10" y="20" fill="#a3e635" fontSize="11">f(x) = x{'²'}</text>
          <text x="10" y="35" fill="#f59e0b" fontSize="11">f'(x) = 2x</text>
        </svg>
      ),
    },
    chem: {
      desc: 'Perform a virtual acid-base titration. HCl + NaOH → NaCl + H₂O',
      visual: (
    ...
    <div className="p-4 h-full flex flex-col gap-4">
      {content ? (
        <>
          ...
          <div className="flex gap-2">
            <button onClick={() => setStep(s => Math.max(0, s - 1))}
              disabled={step === 0} className="btn-secondary text-xs px-3 py-1.5 disabled:opacity-30 gap-1"><ChevronLeft size={12} /> Step</button>
            <button onClick={() => setStep(s => Math.min(4, s + 1))}
              disabled={step === 4} className="btn-primary text-xs px-3 py-1.5 disabled:opacity-30 gap-1">Step <ChevronRight size={12} /></button>
            <span className="text-xs text-white/30 self-center ml-2">Step {step + 1} / 5</span>
          </div>
        </>
      ) : (
        <div className="flex items-center justify-center h-full flex-col gap-3">
          ...
          <p className="text-xs text-white/30">Interactive simulation for this domain coming soon</p>
        </div>
      )}
    </div>
  )
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col animate-fade-in" style={{ background: '#0a0b0f' }}>
      ...
      <div className="h-12 border-b border-[#1e2028] flex items-center justify-between px-4 flex-shrink-0" style={{ background: 'rgba(10,11,15,0.97)' }}>
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-white/35 flex items-center gap-1"><Clock size={11} /> {lab.duration}</span>
          ...
          <div className="p-4 flex-1">
            {activePanel === 'instructions' && (
              <div className="space-y-2">
                <p className="text-xs text-white/40 mb-3">Follow each step in order. Check them off as you go.</p>
                ...
                <div className="flex gap-2 pt-3">
                  <button onClick={() => setCurrentStep(s => Math.max(0, s - 1))}
                    disabled={currentStep === 0} className="btn-secondary text-xs py-1.5 px-3 disabled:opacity-30 gap-1"><ChevronLeft size={12} /> Prev</button>
                  <button onClick={() => setCurrentStep(s => Math.min(lab.steps.length - 1, s + 1))}
                    disabled={currentStep === lab.steps.length - 1} className="btn-primary text-xs py-1.5 px-3 disabled:opacity-30 gap-1">Next <ChevronRight size={12} /></button>
                </div>
              </div>
            )}
            ...
            {activePanel === 'objective' && (
              <div>
                ...
                <div className="mt-4">
                  <p className="text-xs text-white/30 font-semibold mb-2 uppercase">Technologies</p>
            ...
            {activePanel === 'reflect' && (
              <div>
                ...
                <p className="text-xs text-white/40 mb-3">Answer to complete the lab and earn your badge.</p>
...
export default function VirtualLabs() {
  ...
  const domains  = [{ id: 'all', label: 'All Labs', icon: null }, ...MAJORS.filter(m => VIRTUAL_LABS.some(l => l.domain === m.id))]
  ...
  const complete = (labId) => {
    ...
    showNotification('Lab complete! Badge earned.')
  }
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 animate-fade-in">
      ...
      <div className="mb-8">
        <span className="flex items-center gap-1.5 mb-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#a3e635]">
          <FlaskConical size={12} />Virtual Labs
        </span>
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        <div className="card text-center">
          ...
          <p className="text-xs text-white/55">Available Labs</p>
        </div>
        <div className="card text-center">
        ...
        <div className="card text-center">
          ...
          <p className="text-xs text-white/55">Remaining</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
        {domains.map(d => (
          <button key={d.id} onClick={() => setDomainFilter(d.id)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium flex-shrink-0 transition-colors border ${
              domainFilter === d.id
                ? 'bg-[#a3e635]/12 text-[#a3e635] border-[#a3e635]/30'
                : 'text-white/55 hover:text-white border-[#1e2028] hover:bg-white/[0.04]'
            }`}>
            {d.id === 'all' ? <FlaskConical size={13} /> : <span>{d.icon}</span>} {d.label || d.id}
          </button>
        ))}
      </div>
